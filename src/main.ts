...
const MAPLIBRE_STYLE_ID = "odyssey-maplibre-css";
...
export default class OdysseyPlugin extends Plugin {
  ...
  async onload() {
    ...
    this.addCommand({
      id: "open-map",
      name: "Open Map",
      callback: () => this.activateView(VIEW_TYPE_MAP),
    });
    ...
    this.addRibbonIcon("map", "Odyssey Map", () => {
      this.activateView(VIEW_TYPE_MAP);
    });
  }

  async onunload() {
    document.getElementById(MAPLIBRE_STYLE_ID)?.remove();
  }

  private injectMaplibreCss() {
    if (document.getElementById(MAPLIBRE_STYLE_ID)) return;
    const style = document.createElement("style");
    style.id = MAPLIBRE_STYLE_ID;
    style.textContent = maplibreCssText;
