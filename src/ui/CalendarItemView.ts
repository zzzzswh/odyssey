...
export class CalendarItemView extends ItemView {
  ...
  private render() {
    ...
    this.root.render(
      React.createElement(CalendarView, {
        ...
        onSwitchToMap: () => this.switchTo(VIEW_TYPE_MAP),
        onSplitWithMap: () => this.splitWith(VIEW_TYPE_MAP),
      }),
    );
  }
  ...
  private async deleteEntry(entry: Entry) {
    const file = entry.file;
    if (!(file instanceof TFile)) return;
    new DeleteEntryModal(this.app, entry, async () => {
      // Route through system trash; user can recover from OS trash.
